...
PRJ_ONBOARD=onboarding-dev-$PRJ_SUFFIX
...
function print_info() {
  ...
  echo "GitHub repo:         https://github.com/$GITHUB_ACCOUNT/test"
...
function delete_projects() {
  oc delete project $PRJ_COOLSTORE_TEST $PRJ_DEVELOPER $PRJ_COOLSTORE_PROD $PRJ_INVENTORY $PRJ_ONBOARD $PRJ_CI
}
...
function create_projects() {
  ...
  echo "Creating project $PRJ_ONBOARD"
  oc new-project $PRJ_ONBOARD --display-name='Inventory TEST' --description='Onboard Test Environment' >/dev/null
  ...
  for project in $PRJ_CI $PRJ_COOLSTORE_TEST $PRJ_COOLSTORE_PROD $PRJ_INVENTORY $PRJ_ONBOARD $PRJ_DEVELOPER
  do
  ...
  if [ $LOGGEDIN_USER == 'system:admin' ] ; then
    ...
    oc adm pod-network join-projects --to=$PRJ_CI $PRJ_COOLSTORE_TEST $PRJ_DEVELOPER $PRJ_COOLSTORE_PROD $PRJ_INVENTORY $PRJ_ONBOARD >/dev/null 2>&1
  fi
...
# Add Onboard Service Template
function add_onboarding_template_to_projects() {
...
function deploy_gogs() {
  ...
  local _TEMPLATE="https://raw.githubusercontent.com/lapd-viosoft/gogs-openshift-docker/binary/openshift/gogs-persistent-template.yaml"
  if [ "$ARG_EPHEMERAL" = true ] ; then
    _TEMPLATE="https://raw.githubusercontent.com/lapd-viosoft/gogs-openshift-docker/binary/openshift/gogs-template.yaml"
  fi
...
EOM
  ...
  local _CLONE_DIR=/tmp/$(date +%s)-test
  rm -rf $_CLONE_DIR && \
      git clone http://$GOGS_ROUTE/$GOGS_ADMIN_USER/test.git $_CLONE_DIR && \
      cd $_CLONE_DIR && \
      git branch -m master master-old && \
      git checkout $GITHUB_REF && \
      git branch -m $GITHUB_REF master && \
      git push -f http://$GOGS_ADMIN_USER:$GOGS_ADMIN_PASSWORD@$GOGS_ROUTE/$GOGS_ADMIN_USER/test.git master && \
      rm -rf $_CLONE_DIR
}
...
function deploy_coolstore_test_env() {
  ...
  if [ "$ARG_MINIMAL" == true ] ; then
    scale_down_deployments $PRJ_COOLSTORE_TEST onboarding
  fi
}
...
function deploy_onboarding_dev_env() {
  ...
  echo "Using template $_TEMPLATE"
  oc process -f $_TEMPLATE -v GIT_URI=http://$GOGS_ROUTE/$GOGS_ADMIN_USER/test.git -v MAVEN_MIRROR_URL=$MAVEN_MIRROR_URL -n $PRJ_INVENTORY | oc create -f - -n $PRJ_INVENTORY
  sleep 2
  # scale down to zero if minimal
  if [ "$ARG_MINIMAL" = true ] ; then
    scale_down_deployments $PRJ_INVENTORY onboarding
  fi
}
...
function build_images() {
  ...
  # build images
  #for buildconfig in web-ui inventory onboarding cart catalog coolstore-gw pricing
  for buildconfig in onboarding
  do
...
function promote_images() {
  ...
  oc tag $PRJ_COOLSTORE_TEST/onboarding:latest $PRJ_INVENTORY/onboarding:latest
  oc tag $PRJ_COOLSTORE_TEST/onboarding:latest $PRJ_COOLSTORE_TEST/onboarding:test
  oc tag $PRJ_COOLSTORE_TEST/onboarding:latest $PRJ_COOLSTORE_PROD/onboarding:prod-green
  oc tag $PRJ_COOLSTORE_TEST/onboarding:latest $PRJ_COOLSTORE_PROD/onboarding:prod-blue
  oc tag $PRJ_COOLSTORE_TEST/onboarding:latest -d
...
EOM
  ...
  _RETURN=$(curl -o /dev/null -sL -w "%{http_code}" -H "Content-Type: application/json" -d "$_DATA_JSON" -u $GOGS_ADMIN_USER:$GOGS_ADMIN_PASSWORD -X POST http://$GOGS_ROUTE/api/v1/repos/$GOGS_ADMIN_USER/test/hooks)
  if [ $_RETURN != "201" ] && [ $_RETURN != "200" ] ; then
...
case "$ARG_COMMAND" in
    ...
    *)
        ...
        deploy_gogs
        deploy_jenkins
        add_onboarding_template_to_projects
        deploy_coolstore_test_env
        deploy_coolstore_prod_env
        deploy_onboarding_dev_env
        promote_images
        deploy_pipeline
